    ...
    class ProductPage : Fragment(),SearchView.OnQueryTextListener{
                ...
                productPageViewModel.getProductList().observe(viewLifecycleOwner){productlist->
                    favoritiesPageViewModel.getFavoriteList().observe(viewLifecycleOwner){favoirteList->
                     filterOptions.observe(viewLifecycleOwner){filters->
                         searchQuery.observe(viewLifecycleOwner){searchText->
                             Log.d("hatamSearchText","srt : "+searchText)
                             if(searchText!=null && searchText!=""){
                                 adapter = ProductPageAdapter(requireContext(), sortProducts(sortingMode,filter(filters,getListWithSearch(productlist,searchText))) ,this,favoirteList)

                             }else{
                                 adapter = ProductPageAdapter(requireContext(), sortProducts(sortingMode,filter(filters,productlist)) ,this,favoirteList)
        ...
        fun getListWithSearch(list: ArrayList<Product>, query:String ):ArrayList<Product>{

            return list.filter {
                val productName = it.product_name.orEmpty().lowercase()
                val productInfo = it.product_info.orEmpty().lowercase()
                val searchQuery = query.lowercase()
                productName.contains(searchQuery) || productInfo.contains(searchQuery)} as ArrayList<Product>
        }
